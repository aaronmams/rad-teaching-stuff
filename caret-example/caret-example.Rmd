...
These data come from a satellite monitoring system that tracks the location of commercial fishing vessels in the U.S.  These data are logged on hourly intervals and record a vessels location in lat/long coordinates and a time stamp.  
...
df <- df %>% filter(month(local_time) %in% c(6,7,8,9,10)) %>% 
  select(fishing,speed,bottom_depth,len,hour,bearing.rad) %>%
  mutate(y=fishing) %>% select(-fishing)
...
"Fishing" observations are about 27% of the training and testing data...so that's cool.
...
df$fold <- createFolds(df.est$y,k=5,list=F)
head(df)
df <- df %>% select(-fold)
```
...
The real red meat of the caret library is the *train()* function.  *train()* works with [238 models](http://topepo.github.io/caret/available-models.html) from a variety of different R libraries.  In the chunk below we use the preprocessed training data to train a neural network model using the [nnet](https://cran.r-project.org/web/packages/nnet/nnet.pdf) library.
